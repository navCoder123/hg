              ...
              {orders.map((order) => {
                ...
                return (
                  <div key={order._id} className="bg-neutral-900 border border-gray-700 rounded-xl p-5 shadow-lg">
                    ...
                    <p className="text-gray-400 mb-1"><strong>Date:</strong> {formattedDate}</p>
                    <p className="text-gray-400 mb-1"><strong>Location:</strong> {event?.location || "N/A"}</p>
                    <p className="text-gray-400 mb-1"><strong>Amount:</strong> ₹{order.amount?.toLocaleString() || "N/A"}</p>
                    <p className="text-gray-400 mb-1"><strong>Ticket ID:</strong> {order.paymentId || "N/A"}</p>
                    ...
                    {order.qrDataUrl && (
                      <div className="mt-4 text-center">
                        <img src={order.qrDataUrl} alt="Ticket QR" className="w-32 h-32 object-contain mx-auto mb-2" />
                        <a href={order.qrDataUrl} download={`ticket-${order._id}.png`} className="text-purple-400 text-sm underline hover:text-purple-300">
                          Download Ticket
                        </a>
                      </div>
                    )}
